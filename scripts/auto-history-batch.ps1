...
#   - 🔧 從 master-articles.json 動態讀取歷史文章
#   - 🔧 符合血淚教訓 #9
...
Write-Log "============================================================"
Write-Log "📜 歷史腦洞 — 即時顯示 + 排程優化版 v11.0"
...
# ============================================================
# 步驟 2：從 master-articles.json 讀取歷史文章
# ============================================================
Write-Log "[2/5] 從 master-articles.json 讀取歷史文章..."
...
# 找出所有 history/ 開頭的文章
history_articles = [a for a in articles if a.get('filename', '').startswith('history/')]

print(f'📊 找到 {len(history_articles)} 篇歷史文章')

# 檢查哪些已存在
missing = []
for a in history_articles:
...
if ($MissingCount -eq 0) {
    Write-Log "   ✅ 所有歷史文章已存在，無需生成"
    Write-Log "============================================================"
    Write-Log "✅ 檢查完成！所有文章已存在"
    exit 0
}

Write-Log "   ⚠️ 缺失 $MissingCount 篇歷史文章，開始生成..."
...
for idx, article in enumerate(history_articles, 1):
    ...
    if os.path.exists(filepath):
        ...
        if size >= 5120:
            print(f'⏩ [{idx}/{len(history_articles)}] 已存在：{filename} ({size} bytes)')
            skip_count += 1
            continue
        else:
            print(f'⚠️ [{idx}/{len(history_articles)}] 檔案過小，重新生成：{filename} ({size} bytes)')
    
    print(f'--- 進度 {idx}/{len(history_articles)} ---')
    try:
...
Write-Log "============================================================"
Write-Log "✅ 歷史腦洞 — 即時顯示 + 排程優化版 v11.0 執行完畢！"
...
if ($SmtpUser -and $SmtpPass -and $ToEmail) {
    $Subject = "📜 歷史腦洞批次生成完成通知"
    $Body = @"
歷史腦洞批次生成已於 $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') 完成！

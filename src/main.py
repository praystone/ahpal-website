...
from src.content_router import generate_content  # 🆕 使用路由
...
keywords_list = [
    ...
    # ============================================================
    {
        ...
        "category": "🎵 音樂創作",
        "filename": "music/bang-chhun-hong-embed.html",
        "video_id": "A9Zw-QHEOqQ",  # 🆕 YouTube 影片 ID
        "content_type": "song"      # 🆕 標記為音樂文章
    },
...
def run_pipeline(force_api=None, dry_run=False):
    """執行內容生成管線（支援文章 + 音樂）"""
    logger.info("=" * 70)
    logger.info(f"🦞 AHPAL.COM 內容生成引擎 v5.2 - {CURRENT_YEAR}")
    ...
    if dry_run:
        ...
        for item in pending_items:
            ...
            logger.info(f"   - [{content_type}] {item['keyword']} ({item['category']})")
...
def main():
    parser = argparse.ArgumentParser(description='AHPAL 內容生成引擎 v5.2')
